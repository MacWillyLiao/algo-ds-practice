...
## 讀檔格式
測資放在 [`input/`](parallel_sorting_prac/data/input/) 資料夾中，例如：
- 一萬筆資料：[input_1w.txt](parallel_sorting_prac/data/input/input_1w.txt)
- 十萬筆資料：[input_10w.txt](parallel_sorting_prac/data/input/input_10w.txt)
- 五十萬筆資料：[input_50w.txt](parallel_sorting_prac/data/input/input_50w.txt)
- 一百萬筆資料：[input_100w.txt](parallel_sorting_prac/data/input/input_100w.txt)

## 寫檔格式
Output 檔名格式：{ Input File Name }_output{ Method Number }.txt，如 [`output/`](parallel_sorting_prac/data/output/) 資料夾中的檔案，此資料夾有所有資料筆數及方法的輸出。每個檔案有排序完的資料、執行時間和 Output Time，如以下：
```
Sort :
8
12
...
## 程式執行方式
[main.py](parallel_sorting_prac/main.py) 編譯並執行後會提供輸入
- 範例一：不需要輸入 K  
method: 1
    ```
...
## 結果
詳細書面報告在 [report.pdf](parallel_sorting_prac/report.pdf) 中
